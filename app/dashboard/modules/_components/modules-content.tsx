...
import { useStaffPortalModules } from "@/hooks/use-staff-portal";

export default function ModulesContent() {
  const { data: modules, isLoading } = useStaffPortalModules();
  ...
  return (
    <div className="h-[calc(100vh-86px)] overflow-y-auto px-6 pt-4 pb-10 custom-scrollbar">
      <div className="space-y-5 max-w-[1700px] mx-auto">
        <div>
          <h1 className="text-xl font-bold text-gray-800 dark:text-gray-100">Modules</h1>
          <p className="text-sm text-gray-500 mt-0.5">System modules and their permissions (read-only)</p>
        </div>
