...
import {
  useCreateStaffPortalClient,
  useUpdateStaffPortalClient,
  useStaffPortalClient,
} from "@/hooks/use-staff-portal";
...
export function AddClientContent({
  ...
  const { data: fetchedClient } = useStaffPortalClient(
    isEdit || isView ? id : undefined,
  );
  const effectiveData = initialData || fetchedClient;

  const createMutation = useCreateStaffPortalClient();
  const updateMutation = useUpdateStaffPortalClient();
  ...
  const [registrationType, setRegistrationType] = useState<"guest" | "client">(
    initialData?.registration_type || "client",
  );
  ...
  const [sendCredentialsToEmail, setSendCredentialsToEmail] = useState<boolean>(
    Number(initialData?.send_credentials_to_email) === 1 ||
      initialData?.send_credentials_to_email === true,
  );
  ...
  useEffect(() => {
    if (effectiveData) {
      ...
      setRegistrationType(effectiveData.registration_type || "client");
      setSelectedPlan(effectiveData.plan || "");
      setLoginAccess(
        Number(effectiveData.login_access) === 1 ||
          effectiveData.login_access === true,
      );
      setSendCredentialsToEmail(
        Number(effectiveData.send_credentials_to_email) === 1 ||
          effectiveData.send_credentials_to_email === true,
      );
    }
  }, [effectiveData]);
              ...
              <div className="pt-4 border-t border-gray-100 dark:border-gray-800 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      !isView && setSendCredentialsToEmail(checked)
                    }
                    disabled={isView}
                    className="transition-all"
                  />
