...
import { useStaffPortalClients, useDeleteStaffPortalClient, useUpdateClientStatus, useUpdateClientLoginAccess, StaffPortalClient } from "@/hooks/use-staff-portal";
...
type SortKeys = keyof StaffPortalClient;
...
export default function ClientsListContent() {
  ...
  // Modals state
  const [clientToDelete, setClientToDelete] = useState<StaffPortalClient | null>(null);

  // Fetch data via hook
  const { data: clientsRes, isLoading: loading } = useStaffPortalClients({
    page: currentPage,
    limit: itemsPerPage,
    search: searchQuery,
  ...
  const deleteMutation = useDeleteStaffPortalClient();
  const updateStatusMutation = useUpdateClientStatus();
  const updateLoginAccessMutation = useUpdateClientLoginAccess();
  ...
  const handleView = (client: StaffPortalClient) => {
              ...
              <tr className="bg-gray-50/50 dark:bg-gray-800/20 border-b border-gray-100/50 dark:border-gray-800">
                {[
                  ...
                  <th 
                    ...
                    className="px-6 py-5 text-[10px] font-extrabold text-gray-400 uppercase tracking-widest cursor-pointer group active:opacity-70 select-none"
                  >
                  ...
                  <tr key={client.id} className="group hover:bg-blue-50/30 dark:hover:bg-blue-500/[0.03] transition-all relative overflow-hidden">
                      ...
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-3">
                          <Switch
                            ...
                            onCheckedChange={() => updateStatusMutation.mutate({ id: client.id, is_active: client.is_active === 1 ? 0 : 1 })}
                    ...
                    {visibleColumns.includes("login_access") && (
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-3">
                          <Switch
                            checked={!!client.login_access}
                            onCheckedChange={() => updateLoginAccessMutation.mutate({ id: client.id, login_access: client.login_access ? 0 : 1 })}
                            disabled={updateLoginAccessMutation.isPending}
                            className="transition-all"
                          />
                          <span className="text-[12px] font-semibold text-gray-600 dark:text-gray-400">
                            {client.login_access ? 'Allowed' : 'Denied'}
                          </span>
                        </div>
                      </td>
                    )}
