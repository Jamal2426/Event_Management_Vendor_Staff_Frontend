...
export default function RoleForm({ role }: RoleFormProps) {
  ...
  const toggleAll = (checked: boolean) => {
    ...
    const allIds = safeModules.flatMap((m) => m.permissions.map((p) => p.id));
  ...
  const allPermissionIds = safeModules.flatMap((m) => m.permissions.map((p) => p.id));
        ...
        <div className="flex items-center justify-between mb-4 pb-3 border-b border-gray-100 dark:border-gray-800">
          ...
          <div className="flex items-center gap-3 text-xs">
            ...
            <button
              ...
              onClick={() => setExpandedModules(new Set(safeModules.map((m) => m.slug)))}
            >
              Expand all
            </button>
          </div>
        ...
        {modulesLoading ? (
          ...
          <div className="space-y-2">
            {safeModules.map((mod) => {
              ...
              const modPermIds = Array.isArray(mod.permissions) ? mod.permissions.map((p) => p.id) : [];
                ...
                <div key={mod.id} className="rounded-2xl border border-gray-100 dark:border-gray-800 overflow-hidden">
                  ...
                  >
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => Array.isArray(mod.permissions) && toggleModuleAll(mod.permissions, !!checked)}
                  ...
                  {isExpanded && (
                    <div className="px-6 py-3 flex flex-wrap gap-x-6 gap-y-2">
                      {Array.isArray(mod.permissions) && mod.permissions.map((perm) => {
