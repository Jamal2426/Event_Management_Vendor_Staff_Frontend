...
import { useStaffPortalStaff, useDeleteStaffPortalStaffMember, StaffPortalStaff } from "@/hooks/use-staff-portal";

type SortKeys = keyof StaffPortalStaff;
...
export default function StaffListContent() {
  ...
  // Modals state
  const [employeeToDelete, setEmployeeToDelete] = useState<StaffPortalStaff | null>(null);

  // Fetch data via hook
  const { data: staffRes, isLoading: loading } = useStaffPortalStaff({
    page: currentPage,
    limit: itemsPerPage,
    search: searchQuery,
  ...
  const deleteMutation = useDeleteStaffPortalStaffMember();
  ...
  const handleView = (emp: StaffPortalStaff) => {
    router.push(`/dashboard/staff/view/${emp.id}`);
  };
            ...
            <thead className="sticky top-0 z-20 bg-white dark:bg-[#1f2937] shadow-sm">
              <tr className="bg-gray-50/50 dark:bg-gray-800/20 border-b border-gray-100/50 dark:border-gray-800">
                {[
                  ...
                  { key: "mobile", label: "Mobile"},
                  { key: "doj", label: "DOJ"},
                  { key: "login_access", label: "Access", align: "center" },
                  { key: "is_active", label: "Status"},
                    ...
                    {visibleColumns.includes("doj") && (
                      <td className="px-6 py-5">
                         <div className="flex items-center gap-2">
                            ...
                            <p className="text-[13px] font-bold text-gray-500 dark:text-gray-400">{emp.doj ? new Date(emp.doj).toLocaleDateString() : '—'}</p>
                         </div>
                      </td>
                    )}
