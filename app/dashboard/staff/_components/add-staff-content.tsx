...
import { useCreateVendorStaff, useUpdateVendorStaff, useStaffPortalStaffById, useStaffPortalRoles, StaffPortalRole } from "@/hooks/use-staff-portal";
...
export default function AddStaffContent({ initialData, isEdit = false, isView = false }: AddStaffContentProps) {
  ...
  const { data: memberData } = useStaffPortalStaffById(id || "");
  const { data: rolesData } = useStaffPortalRoles({ limit: 100 });
  ...
  const validateForm = () => {
    ...
    if (!id && !formData.role_id) newErrors.designation = "Designation is required";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (id) {
      // Only send whitelisted fields on edit — login_access, role_id, password are vendor-only
      const { login_access, role_id, password, ...editableFields } = formData;
      await updateMutation.mutateAsync({ id: parseInt(id), data: { ...editableFields, profile_pic: profilePic } });
    } else {
    ...
    <div className="h-[calc(100vh-86px)] overflow-y-auto px-4 sm:px-6 lg:px-8 pt-2 pb-10 custom-scrollbar">
        ...
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-4 mt-4">
             <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100 uppercase tracking-tight flex items-center gap-2">
                {params.id ? (isView ? "EMPLOYEE DETAILS" : "EDIT EMPLOYEE") : "ADD STAFF"}
             </h1>
          ...
          <p className="text-sm font-medium text-gray-400">Enter the employee&apos;s professional details to manage their profile.</p>
        </div>
      ...
      <div className={`flex flex-col lg:flex-row gap-8 ${!isView ? "pb-20" : ""}`}>
        ...
        <div className={`flex-[2] space-y-6`}>
           ...
           >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                   ...
                   >
                    ...
                    {/* Mobile */}
                    <FormGroup 
                      label="Mobile Number" icon={Phone} error={errors.mobile} required isView={isView}
                    >
                          <Input 
                    ...
                    {/* DOB */}
                    <FormGroup 
                      label="Date of Birth" icon={Calendar} error={errors.dob} required isView={isView}
                    >
                          <Input 
                            type="date"
                            value={formData.dob}
                            onChange={(e) => {
                              !isView && setFormData({ ...formData, dob: e.target.value });
           ...
           >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    {/* Designation — editable only on create; role changes are vendor-only */}
                    <FormGroup
                      label="Designation" icon={Briefcase} error={errors.designation} required={!id} isView={isView || !!id}
                    >
                      {(isView || id) ? (
                        <Input
                        ...
                        <Select
                          ...
                          onValueChange={(val) => {
                            const selectedRole = rolesData?.find((r: StaffPortalRole) => String(r.id) === val);
                    ...
                    >
                          <Input
                            ...
                            value={formData.doj}
                            onChange={(e) => {
                              !isView && setFormData({ ...formData, doj: e.target.value });
           ...
           >
                <div className="space-y-6">
                    <FormGroup 
                      label="Street Address" icon={Home} error={errors.address} required isView={isView}
                    >
                          <textarea 
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                       {/* Country */}
                       <FormGroup 
                          label="Country" required isView={isView} error={errors.country}
                       >
                          <SearchableSelect 
                       ...
                       {/* State */}
                       <FormGroup 
                          label="State" required isView={isView} error={errors.state}
                       >
                          <SearchableSelect 
                       ...
                       {/* District */}
                       <FormGroup 
                          label="District" required isView={isView} error={errors.district}
                       >
                          <SearchableSelect 
                       ...
                       {/* City */}
                       <FormGroup 
                          label="City" required isView={isView} error={errors.city}
                       >
                          <SearchableSelect 
                             value={formData.city} 
                             onValueChange={(val) => {
                       ...
                       {/* Locality */}
                       <FormGroup 
                          label="Locality" icon={MapPin} error={errors.locality} required isView={isView}
                       >
                             <Input 
                       ...
                       {/* Pincode */}
                       <FormGroup 
                          label="Pincode" icon={Hash} error={errors.pincode} required isView={isView}
                       >
                             <Input 
        ...
        <div className="flex-1 space-y-8">
           ...
           <div className="bg-white dark:bg-gray-800/40 rounded-3xl border border-gray-100 dark:border-gray-800 p-8 shadow-sm text-center">
              ...
              <p className="text-[10px] text-gray-400 font-bold uppercase tracking-tight mt-2 italic">MAX 10MB</p>
           </div>
           ...
           <div className="bg-white dark:bg-gray-800/40 rounded-3xl border border-gray-100 dark:border-gray-800 p-8 shadow-sm space-y-8">
              ...
              <div className="space-y-4">
                 <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-[12px] font-black text-gray-800 dark:text-gray-100 uppercase tracking-[0.2em]">LOGIN ACCESS</h3>
                 </div>
                 ...
                 <div className="grid grid-cols-2 gap-2 p-1.5 bg-gray-50/50 dark:bg-gray-900/50 rounded-2xl border border-gray-100 dark:border-gray-800">
                    <button
                      type="button"
                      onClick={() => (!isView && !id) && setFormData({...formData, login_access: true})}
                      className={`flex items-center justify-center gap-2 py-2.5 rounded-xl text-[11px] font-bold transition-all duration-300 ${formData.login_access ? "bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg shadow-blue-500/20" : "text-gray-400"} ${(isView || id) ? "cursor-default opacity-60" : "hover:shadow-blue-500/40 hover:-translate-y-0.5 hover:brightness-110 active:scale-95"}`}
                    >
                       <ShieldCheck size={14} /> Allow
                    </button>
                    <button
                      type="button"
                      onClick={() => (!isView && !id) && setFormData({...formData, login_access: false})}
                      className={`flex items-center justify-center gap-2 py-2.5 rounded-xl text-[11px] font-bold transition-all duration-300 ${!formData.login_access ? "bg-rose-500 text-white shadow-lg shadow-rose-500/20" : "text-gray-400"} ${(isView || id) ? "cursor-default opacity-60" : "hover:shadow-rose-500/40 hover:-translate-y-0.5 hover:brightness-110 active:scale-95"}`}
                    >
