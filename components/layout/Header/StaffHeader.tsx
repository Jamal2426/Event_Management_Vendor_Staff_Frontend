...
import {
  faSearch, faExpand, faCompress, faMoon, faSun, faBell, faCheckDouble,
...
import { useStaffMe, useStaffLogout } from "@/hooks/use-staff-auth";
    ...
    <header className="h-[56px] pl-3 pr-6 flex items-center justify-between bg-card dark:bg-[#09090b] backdrop-blur-md border-b border-border dark:border-[#27272a] sticky top-0 z-50 min-w-0 overflow-hidden text-foreground dark:text-gray-100">
      ...
      <div className="flex items-center justify-end gap-1.5 md:gap-2 shrink-0">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-[300px] p-0 border-border rounded-2xl shadow-xl mt-2 overflow-hidden">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-[240px] p-0 rounded-lg shadow-xl mt-2 border-border overflow-hidden">
            <div className="bg-primary p-5 text-center">
              <h6 className="font-bold text-[15px] text-white leading-tight uppercase tracking-wide">{staff?.name ?? "Staff"}</h6>
              <span className="text-[11px] text-white/80 font-normal pt-1 block">{staff?.designation ?? "Staff Member"}</span>
            </div>
            <div className="p-1.5 bg-card">
              <DropdownMenuItem asChild className="flex items-center h-11 px-4 cursor-pointer text-foreground text-[13px] font-medium hover:bg-accent focus:bg-accent outline-none group border-b border-border/50 rounded-none first:rounded-t-md">
                <Link href="/dashboard/profile">
                  <User size={16} className="mr-3 text-muted-foreground group-hover:text-primary" /> My Profile
                </Link>
              </DropdownMenuItem>
              <DropdownMenuItem asChild className="flex items-center h-11 px-4 cursor-pointer text-foreground text-[13px] font-medium hover:bg-accent focus:bg-accent outline-none group border-b border-border/50 rounded-none">
                <Link href="/dashboard/settings/config">
                  <Settings size={16} className="mr-3 text-muted-foreground group-hover:text-primary" /> Settings
                </Link>
              </DropdownMenuItem>
              <DropdownMenuItem
