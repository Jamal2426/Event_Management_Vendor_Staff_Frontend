          ...
          <thead className="sticky top-0 z-20 bg-white dark:bg-[#1f2937] shadow-sm">
            <tr className="bg-gray-50/50 dark:bg-gray-800/20 border-b border-gray-100/50 dark:border-gray-800">
              ...
              {filteredColumns.map((col) => (
                ...
                >
                  <div className={cn("flex items-center gap-2 transition-colors group-hover:text-gray-600", col.align === "center" ? "justify-center" : "")}>
                    {col.label}
                    {col.sortable && (
                      <ArrowUpDown size={12} className={cn("transition-all", sortConfig?.key === col.key ? "text-gray-600 opacity-100" : "opacity-30 group-hover:opacity-60")} />
                    )}
                  </div>
                </th>
              ))}
